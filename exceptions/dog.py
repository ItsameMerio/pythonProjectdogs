i# DO NOT CHANGE THE IMPORTS. OTHERWISE THE GRADING IN A+ WONT WORK
...
class Dog:
    ...
    def feed(self, food_given):
        ...
        if not isinstance(food_given, Food):
            raise FeedingDogErrorNotFoodObject(food_given)
        elif len(self.foods)==0:
            raise FeedingDogErrorMissingList(food_given)
        for food in self.foods:
            if not isinstance(food, Food):
                raise FeedingDogErrorObjectInListNotFoodObject(food)
